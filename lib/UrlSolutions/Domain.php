...
class Domain extends UrlSolutionsApiClient
{
	...
	const TRANSFER_IN_COMPLETED_STATUS = 'Completed';

	const PENDING_TRANSFER_IN_STATUS_LIST = [
		'waiting registrant confirmation',
		'waiting transfer'
	];

	const REJECTED_TRANSFER_IN_STATUS_LIST = [
		'registrant rejected',
		'registrar rejected'
	];

	const DOMAIN_REDEMPTION_STATUS = 'redemption';
	const DOMAIN_TRANSFER_OUT_READY_STATUS = 'transfer out ready';
	const DOMAIN_OK_STATUS = 'ok';
	const DOMAIN_TRANSFERRING_OUT_STATUS = 'transferring out';
	const DOMAIN_EXPIRED_STATUS = 'expired';
	const DOMAIN_SUSPENDED_STATUS = 'suspended';
	...
	public function getTransferStatus($domain)
	{
		...
		if (in_array($transferStatus, self::PENDING_TRANSFER_IN_STATUS_LIST)) {
			$status['pending'] = true;

		} else if (in_array($transferStatus, self::REJECTED_TRANSFER_IN_STATUS_LIST)) {
			$status['rejected'] = true;

		} else if (empty($transferStatus) && !empty($domainsList['data'])) {
			$status['description'] = self::TRANSFER_IN_COMPLETED_STATUS;
			$status['success'] = true;
		}
	...
	public function requestDelete($domain)
	{
		if ($this->isAvailable($domain)) {
			throw new \Exception('Domain is not registered.');
		}

		return parent::delete("/domains/{$domain}", [], __FUNCTION__);
	}

	public function redeem($domain)
	{
		return parent::put("/domains/{$domain}/redeem", [], __FUNCTION__);
	}

	public function createChildNameserver($domain, $hostname, $ipv4 = null, $ipv6 = null)
	{
		if (empty($hostname)) {
		...
		return parent::post("/domains/{$domain}/child_name_servers", $data, __FUNCTION__);
	}

	public function updateChildNameserver($domain, $hostname, $ipv4 = null, $ipv6 = null)
	{
		if (empty($hostname)) {
			throw new \Exception('Invalid hostname.');
		}
		...
		return parent::put("/domains/{$domain}/child_name_servers", $data, __FUNCTION__);
	}

	public function getChildNameserver($domain)
	{
		$response = parent::get("/domains/{$domain}/child_name_servers");

		return isset($response['data']) ? $response['data'] : [];
	}

	public function deleteChildNameserver($domain, $hostname)
	{
		if (empty($hostname)) {
			throw new \Exception('Invalid hostname.');
		}

		return parent::delete("/domains/{$domain}/child_name_servers", ['hostname' => $hostname], __FUNCTION__);
	}

    public function getTlds()
    {
        return parent::get("/tlds")['data'];
    }
}
